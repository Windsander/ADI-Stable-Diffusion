...
private:
  void do_statistics(StatisticsLevel level_, const char* msg_) override
  {
    if (notify_duration > NOTIFIER_DURATION_THRESHOLD_MSEC) {
      ...
      log(((loglevel_e)level_)) << "VideoRenderer Statistics: "
...
public:
  explicit target_fps_statistics(StatisticsLevel level_ = CENS_LOG_INFO, const char* message_ = "") : statistics_base(level_, message_)
  {
    // no-action
  }
