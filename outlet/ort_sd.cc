...
static void generate_context(IOrtSDContext_ptr* ctx_pp_, struct IOrtSDConfig ctx_config_){
...
static void released_context(IOrtSDContext_ptr* ctx_pp_){
    if (ctx_pp_ && *ctx_pp_ ){
...
static void init(IOrtSDContext_ptr ctx_p_){
    if (ctx_p_) {
        ((onnx::sd::context::OrtSD_Context *) ctx_p_)->init();
    }
}

static void prepare(IOrtSDContext_ptr ctx_p_, const char* positive_prompts_, const char*negative_prompts_){
    if (ctx_p_) {
...
static IO_IMAGE inference(IOrtSDContext_ptr ctx_p_, IO_IMAGE image_data_){
    if (ctx_p_) {
...
static void release(IOrtSDContext_ptr ctx_p_){
    if (ctx_p_) {
